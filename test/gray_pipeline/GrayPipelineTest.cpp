...
#include "Epub/Epub/converters/AdaptiveTone.h"
...
TEST(GrayPipelineCalibration, DifferentialMatchesLegacyForEveryToneAndBayerCell) {
  for (int gray = 0; gray < 256; ++gray) {
    for (int y = 0; y < 4; ++y) {
      for (int x = 0; x < 4; ++x) {
        const int adjusted = gray + (bayer4x4[y][x] - 8) * 5;
        const int expected = adjusted < 64 ? 0 : adjusted < 128 ? 1 : adjusted < 192 ? 2 : 3;
        EXPECT_EQ(applyBayerDither4Level(gray, x, y, PixelCacheVariant::Differential), expected);
      }
    }
  }
}

TEST(GrayPipelineCalibration, FactoryPreservesHighlightsAndUsesItsOwnMidtones) {
  for (int gray = 250; gray < 256; ++gray) {
    for (int y = 0; y < 4; ++y) {
      for (int x = 0; x < 4; ++x) {
        EXPECT_EQ(applyBayerDither4Level(gray, x, y, PixelCacheVariant::FactoryLut), 3);
      }
    }
  }
  EXPECT_EQ(applyBayerDither4Level(150, 1, 0, PixelCacheVariant::Differential), 2);
  EXPECT_EQ(applyBayerDither4Level(150, 1, 0, PixelCacheVariant::FactoryLut), 1);
}

TEST(GrayPipelineCache, RejectsLegacyCalibrationAndWrongWaveform) {
  const auto differential = PixelCacheVariant::Differential;
  const auto factory = PixelCacheVariant::FactoryLut;
  EXPECT_FALSE(pxcHeaderMatches(480, 0, 0, nullptr));
  EXPECT_FALSE(pxcHeaderMatches(PXC_MAGIC, 1, 0, &differential));
  EXPECT_FALSE(pxcHeaderMatches(PXC_MAGIC, PXC_VERSION, 1, &differential));
  EXPECT_FALSE(pxcHeaderMatches(PXC_MAGIC, PXC_VERSION, 0, &factory));
  EXPECT_FALSE(pxcHeaderMatches(PXC_MAGIC, PXC_VERSION, 42, nullptr));
  EXPECT_TRUE(pxcHeaderMatches(PXC_MAGIC, PXC_VERSION, 0, &differential));
  EXPECT_TRUE(pxcHeaderMatches(PXC_MAGIC, PXC_VERSION, 1, &factory));
  EXPECT_TRUE(pxcHeaderMatches(PXC_MAGIC, PXC_VERSION, 1, nullptr));
  EXPECT_EQ(pxcExpectedSize(5, 3), 14u);
}

TEST(GrayPipelineCalibration, AdaptiveToneStartsAtIdentityAndKeepsMonotonicHighlights) {
  AdaptiveTone tone;
  for (int gray = 0; gray < 256; ++gray) EXPECT_EQ(tone.lut[gray], gray);
  uint32_t histogram[256] = {};
  for (int gray = 60; gray <= 200; ++gray) histogram[gray] = 100;
  ASSERT_TRUE(tone.buildFromHistogram(histogram, 14100));
  for (int gray = 1; gray < 256; ++gray) EXPECT_GE(tone.lut[gray], tone.lut[gray - 1]);
  EXPECT_EQ(tone.lut[255], 255);
  EXPECT_LT(tone.lut[60], 60);
  EXPECT_GT(tone.lut[200], 200);
}
