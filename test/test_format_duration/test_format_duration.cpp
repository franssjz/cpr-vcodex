...
// The formatDurationHm function is self-contained (pure integer math).
// We define a test-local copy matching the implementation in
// src/util/ReadingStatsAnalytics.cpp to verify the algorithm.
...
void test_format_zero_ms() {
  TEST_ASSERT_EQUAL_STRING("0m", ReadingStatsAnalytics::formatDurationHm(0).c_str());
}

void test_format_sub_minute() {
  // 30 seconds = 30000 ms → 0 minutes
  TEST_ASSERT_EQUAL_STRING("0m", ReadingStatsAnalytics::formatDurationHm(30000).c_str());
}

void test_format_exactly_one_minute() {
  TEST_ASSERT_EQUAL_STRING("1m", ReadingStatsAnalytics::formatDurationHm(60000).c_str());
}

void test_format_59_seconds() {
  // 59999ms → 0 minutes (truncation)
  TEST_ASSERT_EQUAL_STRING("0m", ReadingStatsAnalytics::formatDurationHm(59999).c_str());
}

// ── Minutes only (< 1 hour) ────────────────────────────────────────

void test_format_5_minutes() {
  TEST_ASSERT_EQUAL_STRING("5m", ReadingStatsAnalytics::formatDurationHm(5 * 60000ULL).c_str());
}

void test_format_15_minutes() {
  TEST_ASSERT_EQUAL_STRING("15m", ReadingStatsAnalytics::formatDurationHm(15 * 60000ULL).c_str());
}

void test_format_30_minutes() {
  TEST_ASSERT_EQUAL_STRING("30m", ReadingStatsAnalytics::formatDurationHm(30 * 60000ULL).c_str());
}

void test_format_59_minutes() {
  TEST_ASSERT_EQUAL_STRING("59m", ReadingStatsAnalytics::formatDurationHm(59 * 60000ULL).c_str());
}

// ── Hours and minutes ───────────────────────────────────────────────

void test_format_exactly_one_hour() {
  TEST_ASSERT_EQUAL_STRING("1h 0m", ReadingStatsAnalytics::formatDurationHm(60 * 60000ULL).c_str());
}

void test_format_one_hour_30_minutes() {
  TEST_ASSERT_EQUAL_STRING("1h 30m", ReadingStatsAnalytics::formatDurationHm(90 * 60000ULL).c_str());
}

void test_format_2_hours_15_minutes() {
  uint64_t ms = (2 * 60 + 15) * 60000ULL;
  TEST_ASSERT_EQUAL_STRING("2h 15m", ReadingStatsAnalytics::formatDurationHm(ms).c_str());
}

void test_format_10_hours_0_minutes() {
  TEST_ASSERT_EQUAL_STRING("10h 0m", ReadingStatsAnalytics::formatDurationHm(600 * 60000ULL).c_str());
}

void test_format_24_hours() {
  TEST_ASSERT_EQUAL_STRING("24h 0m", ReadingStatsAnalytics::formatDurationHm(1440 * 60000ULL).c_str());
}

void test_format_100_hours() {
  // Large value: 100 hours
  uint64_t ms = 100ULL * 60 * 60000ULL;
  TEST_ASSERT_EQUAL_STRING("100h 0m", ReadingStatsAnalytics::formatDurationHm(ms).c_str());
}

// ── Edge cases ──────────────────────────────────────────────────────

void test_format_1_ms() {
  TEST_ASSERT_EQUAL_STRING("0m", ReadingStatsAnalytics::formatDurationHm(1).c_str());
}
...
void test_format_partial_minutes_truncated() {
  ...
  TEST_ASSERT_EQUAL_STRING("1h 30m", ReadingStatsAnalytics::formatDurationHm(5430000).c_str());
}
