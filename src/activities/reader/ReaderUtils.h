...
inline bool formatCompactDuration(const uint64_t totalMs, char* buf, const size_t bufSize) {
  if (!buf || bufSize < 3 || totalMs == 0) {
    return false;
  }
  constexpr uint64_t MS_PER_MINUTE = 60ULL * 1000ULL;
  auto formatRoundedUnit = [&](const uint64_t value, const char* unit) {
    if (!unit || unit[0] == '\0') {
      return false;
    }
  ...
  if (minutes < 60) {
    return formatRoundedUnit(minutes, tr(STR_ETA_UNIT_MINUTE));
  }
  const uint64_t hours = (minutes + 30) / 60;
  if (hours < 24) {
    return formatRoundedUnit(hours, tr(STR_ETA_UNIT_HOUR));
  }
  const uint64_t days = (hours + 12) / 24;
  if (days < 365) {
    return formatRoundedUnit(days, tr(STR_ETA_UNIT_DAY));
  }
