...
//   2: differential pixels retain the original thresholds and no adaptive tone.
constexpr uint8_t PXC_VERSION = 2;
...
constexpr bool pxcHeaderMatches(const uint16_t magic, const uint8_t version, const uint8_t variant,
                                const PixelCacheVariant* expectedVariant) {
  return magic == PXC_MAGIC && version == PXC_VERSION &&
         variant <= static_cast<uint8_t>(PixelCacheVariant::FactoryLut) &&
         (expectedVariant == nullptr || variant == static_cast<uint8_t>(*expectedVariant));
}
