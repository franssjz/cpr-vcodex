...
inline uint8_t applyBayerDither4Level(uint8_t gray, int x, int y,
                                      PixelCacheVariant variant = PixelCacheVariant::Differential) {
  const bool factory = variant == PixelCacheVariant::FactoryLut;
  int g = gray;
  int offset = factory ? ((g < 64) ? g * 12 / 64 : 12) : 0;
  if (g > 242) offset = 0;
  g -= offset;
  ...
  if (adjusted < (factory ? 150 : 128)) return 1;
  if (adjusted < (factory ? 210 : 192)) return 2;
  return 3;
}
